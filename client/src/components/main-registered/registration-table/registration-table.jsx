import styles from './registration-table.module.css';
...
export function RegistrationTable(props) {
  ...
  const onSubmit = async (values) => {
    ...
    let fullValues = {
      ...
      transport: selectedOptionsTransport,
    ...
    if (userIsActive || userRole === "Admin") {
      const response = await cargoRegistration(fullValues);
      dispatch(addMyCargo(response.data))
    }
  ...
  const validate = (values) => {
    ...
    //   errors.countryfrom = "Заполните поле"
    // }
    // if (!values.countryto) {
    //   errors.countryto = "Заполните поле"
    // }
    if (!values.cityfrom) {
          ...
          <div className={styles.content__inner}>
            <form onSubmit={formik.handleSubmit}>
              ...
              <div className={styles.date_wrap}>
                <DatePicker className={`${styles.input}  ${styles.input__calendar}`} locale={ru} selected={startDate} onChange={(date) => setStartDate(date)} placeholderText={'Дата отгрузки'} id="calendar" />
